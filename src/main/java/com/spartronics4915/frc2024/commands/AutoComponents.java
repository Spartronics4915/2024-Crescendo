...
public class AutoComponents {
        ...
        public static Command groundIntake() {
                ...
                Command realCommand = Commands.sequence(
                ...
                Command simCommand = Commands.sequence(
                                IntakeAssemblyCommands.ComplexSetState(IntakeAssemblyState.GROUNDPICKUP),
                                Commands.print("At target"),
                                IntakeAssemblyCommands.setState(IntakeAssemblyState.LOAD));
        ...
        public static Command loadWhileDrivingThenStopAndShootIfNoteLoaded(Command drivingCommand, TargetDetectorInterface speakerTagDetector) {

                Command startSequenceAndDrive = Commands.parallel(DigestCommands.intakeToLoaded().andThen(Commands.print("INTAKE DONE")),
                                drivingCommand.andThen(Commands.print("SHOOTDRIVE DONE")), Commands.print("StartSequenceAndDrive"))
                                .andThen(Commands.print("START DONE"))
                                .andThen(mSwerve.stopChassisCommand());

                Command tableAutoAim = (new TableAutoAimCommand()).andThen(Commands.waitSeconds(0.5));
                Command aimCommand = Commands.deadline(new LockOnOpenLoopCommand(speakerTagDetector), tableAutoAim);

                Command prepShooterCommands = mShooter.setShooterStateCommand(ShooterState.ON)
                                .alongWith(aimCommand);
                Command waitUntilShotIsDone = RobotBase.isSimulation() ? Commands.waitSeconds(0.5)
                                : Commands.waitUntil(mShooter::beamBreakIsNotTriggered);

                Command turnOffShooter = mShooter.setShooterStateCommand(ShooterState.OFF);
                Timer timer = new Timer();
                Command printTimer = Commands.runOnce(()->System.out.println(timer.get()));
                Command entireSequence = Commands.sequence(
                                ...
                                Commands.print("Prep shooter done"),
                                waitUntilShotIsDone,
                                Commands.runOnce(()->System.out.println(timer.get())),
                                turnOffShooter,
